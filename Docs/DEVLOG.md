...
- **Identify Problem:** The build system was leaving redundant and legacy test binaries 
  ...
  after `make clean`.
- **Root Cause:** The `clean` target in the `Makefile` was only removing binaries listed in 
  the current `TESTS` variable, failing to clear untracked legacy binaries from previous 
  phases.
- **Impact:** Created confusion and clutter in the testing environment.
- **Solution Propose:** Update the `clean` target to perform a wildcard removal of all 
  `test_*` binaries in the `Testing/` directory.

### TECHNICAL SOLUTION
- **Goal/Objective:** Standardize the testing infrastructure and ensure a clean workspace.
- **Phase Related:** Phase 16 (Global Synchronization).
- **Reasoning:** Wildcard removal ensures that any binary generated by the suite is cleared, 
  regardless of its current tracking status in the `Makefile`.
- **Implementation Details:** Modified `Makefile` clean target to: `$(RM) $(TESTS) $(TESTDIR)/test_*
  $(TESTDIR)/*.o`.
